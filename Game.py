...
def drawCurrentBlock(block):
    ...
    for x in block.formationx:
        ...
        if(not isClicked):
            blockrect = pygame.Rect(375 + block.formationx[i] * blockSize, 750 + block.formationy[i] * blockSize, blockSize, blockSize)
...
def drawButton():
    my_font.render_to(screen, (637, 712), f"Reset", (255, 255, 255))
    buttonRect = pygame.Rect(625, 700, 100, 50)
...
def drawBox():
    box = pygame.Rect(275, 600, 250, 250)
    pygame.draw.rect(screen, (255, 255, 255), box, 1)

currentRect = blocks[rand.randint(0, 9)]
run = True
while run:
    ...
    for j in range(8):
        for k in range(8):
            if (map[k][j]):
                rect = pygame.Rect( 200 + j * blockSize, 150 + k * blockSize, blockSize, blockSize)
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            x, y = pygame.mouse.get_pos()
            if(x > 275 and x < 525 and y > 600 and y < 850):
                isClicked = True
            if(x > 625 and x < 725 and y > 700 and y < 750):
                reset()
                score = 0
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and isClicked:
            ...
            if(x > 200 and x < 600 and y > 150 and y < 550):
                i = 0
                for z in currentRect.formationx:
                    ix = (x + currentRect.formationx[i] * blockSize) // sizebtwn - 4
